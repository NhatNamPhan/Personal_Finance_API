...
class AnalyticsService:
    @staticmethod
    def get_spending(user_id: int, start_date: date, end_date: date) -> dict:
        ...
        try:
            with get_db() as cur:
                ...
                rows = cur.fetchall()
                summary = {
                    "total_income": row_summary[1] or 0,
                    "total_expense": row_summary[2] or 0,
                    "net": row_summary[3] or 0
                }
                categories = [{
                    "category_id": row[1],
                    "name": row[2],
    ...
    @staticmethod
    def get_budget_progress(user_id: int, month: date) -> dict:
    ...
    def get_net_worth(user_id: int) -> dict:
        ...
        try:
            with get_db() as cur:
                ...
                return {
                    "user_id": user_id,
                    "net_worth": net_worth[1] if net_worth else 0,
                    "accounts": accounts
                }
        except psycopg2.Error as e:
            raise HTTPException(status_code=500, detail=f"Database error: {e.pgerror}")
